...
export const SwapContextProvider = (props: PropsWithChildren<IInit>) => {
  ...
  useEffect(() => {
    if (typeof window.Titan.onFormUpdate === 'function') {
      window.Titan.onFormUpdate(form);
    }
  }, [form]);

  // onFormUpdate callback
  useEffect(() => {
    if (typeof window.Titan.onScreenUpdate === 'function') {
      window.Titan.onScreenUpdate(screen);
    }
  }, [screen]);
