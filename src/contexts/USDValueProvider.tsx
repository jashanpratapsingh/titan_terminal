...
export const USDValueProvider: FC<PropsWithChildren<IInit>> = ({ children }) => {
  ...
  const [cachedPrices, setCachedPrices] = useLocalStorage<ITokenUSDValue>(`${window.Titan.localStoragePrefix}-cached-token-prices`, {});
  ...
  const { data: tokenPriceMap, isFetched: isLatest } = useQuery<ITokenUSDValue>(
    ...
    async () => {
      ...
      try {
        ...
        console.log('Error fetching prices from Titan Pricing API', error);
      }
      return results;
    },
    {
