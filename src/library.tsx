...
async function loadTitan() {
  ...
  try {
    ...
    console.error(`Error loading Titan Terminal: ${error}`);
    throw new Error(`Error loading Titan Terminal: ${error}`);
  }
}
...
const RenderLoadableTitan = (props: IInit) => {
  ...
  useEffect(() => {
    ...
    if (!loaded) {
      intervalId = setInterval(() => {
        const instance = (window as any).TitanRenderer?.RenderTitan;
        if (instance) {
          setLoaded(true);
        }
      }, 50);
  ...
  const RenderTitan: (props: any) => JSX.Element = useMemo(() => {
    if (loaded) {
      return (window as any).TitanRenderer.RenderTitan;
    }
    ...
    <div className={`fixed ${classes.containerClassName}`} style={classes.style}>
      ...
      >
        {isOpen ? (
          ...
          <TitanLogo width={classes.widgetLogoSize} height={classes.widgetLogoSize} />
        )}
      </div>
...
async function init(passedProps: IInit) {
  ...
  if (restProps.displayMode === 'integrated') {
    ...
    if (!target) {
      throw new Error(`Titan Terminal: document.getElementById cannot find ${integratedTargetId}`);
    }
  ...
  if (enableWalletPassthrough) {
    window.Titan.enableWalletPassthrough = true;
    window.Titan.onRequestConnectWallet = onRequestConnectWallet;
  } else {
    window.Titan.enableWalletPassthrough = false;
  }
  ...
  const stylesheetUrls = [
    ...
    `${scriptDomain}/${bundleName}-Titan.css`,
  ...
  window.Titan.root = root;
  window.Titan._instance = element;

  // Callbacks
  window.Titan.onSwapError = onSwapError;
  window.Titan.onSuccess = onSuccess;
  window.Titan.onFormUpdate = onFormUpdate;
  window.Titan.onScreenUpdate = onScreenUpdate;

  // Special props
  window.Titan.localStoragePrefix = passedProps.localStoragePrefix || 'jupiter-terminal';
}
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && attributes) {
  document.onreadystatechange = function () {
    ...
    if (loadComplete && shouldPreload) {
      setTimeout(() => {
        loadTitan().catch((error) => {
          console.error(`Error pre-loading Titan Terminal: ${error}`);
          throw new Error(`Error pre-loading Titan Terminal: ${error}`);
        });
      }, 2000);
    }
  };
